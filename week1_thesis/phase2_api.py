...
RISK_THESAURUS = {
    ...
    "concern", "pressure",
}                                          # defensible synonyms of risk/uncertainty

RISKW = RISK_THESAURUS                    # <-- change this to test sensitivity
...
rows = []
for p in sorted(glob.glob(os.path.join(TRANSCRIPT_DIR, "XOM_*.txt"))):
    b = os.path.basename(p)
    m = re.search(r"(\d{4})Q([1-4])", b)
    if m:
        year, qn = int(m.group(1)), m.group(2)
    else:
        year, qn = None, None
